...
namespace BTG
{
	...
	public class Player : MonoBehaviour, IDamagable, IDataPersistence, IHealable, IRefuelable
	{
		...
		private void Awake()
		{
			...
			states.Add(State.HeatWave, new PlayerHeatWave(fsm, this, Data, Animator.StringToHash(nameof(State.HeatWave))));
			states.Add(State.GearToss, new PlayerGearTossState(fsm, this, Data, Animator.StringToHash(nameof(State.GearToss))));
			states.Add(State.FireBlaze, new PlayerFireBlazeState(fsm, this, Data, Animator.StringToHash("ChargeUp"))); ///Charges up before plays fireblaze Animation
			states.Add(State.Hit, new PlayerHitState(fsm, this, Data, Animator.StringToHash(nameof(State.Idle)))); //TODO: change to hit (No animation yet)
		...
		private void FixedUpdate()
		{
            ...
            bool inBounds = NavMesh.SamplePosition(transform.position, out NavMeshHit hit, 0.1f, walkMask);
