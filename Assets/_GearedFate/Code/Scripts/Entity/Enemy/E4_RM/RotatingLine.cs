...
namespace BTG
{
    public class RotatingLine : MonoBehaviour
    {
        ...
        {
            Vector2 closestPointOnLine = GetClosestPointOnLine(_line.Points[0] + (Vector2)transform.parent.position, _line.Points[1]+ (Vector2)transform.parent.position, _target.position);
        ...
        {
            Vector2 AB = B - A; // Line direction
            Vector2 AP = P - A; // Vector from A to P

            float t = Vector2.Dot(AP, AB) / Vector2.Dot(AB, AB); // Projection factor
