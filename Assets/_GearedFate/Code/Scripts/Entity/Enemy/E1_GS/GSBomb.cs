...
namespace BTG
{
    public class GSBomb : MonoBehaviour
    {
        ...
        IEnumerator PhysicsThrow()
        {
            const float baseVY = 4f;
            const int baseNumBounces = 3;
            float vY = baseVY;
            int bouncesLeft = baseNumBounces - 1;
            float time = 0f;
            while (gameObject.activeInHierarchy)
            {
                ...
                col.enabled = Animator.transform.localPosition.y < 0.3f && time > 0.5f; // collide near ground but not at the start (the initial throw)
