...
namespace BTG
{
    ...
    public class GSBurrowState : GSBaseState
	{
        ...
        bool isActive = false;
        float initialVolume;
		public GSBurrowState(FiniteStateMachine<GearboundSentinel.State> fsm, GearboundSentinel.State state, GearboundSentinel gs) : base(fsm, state,gs)
		{
		}
        ...
        public override void OnPhysicsUpdate()
		{
            ...
            Vector3 diffToTarget = gearboundSentinel.TargetPlayer.transform.position - gearboundSentinel.transform.position;
		...
		public IEnumerator StartBurrow()
		{
			...
			yield return new WaitForSeconds(gearboundSentinel.Animator.GetCurrentAnimatorClipInfo(0)[0].clip.length - 0.1f);
        ...
        public IEnumerator EndBurrow()
		{
            ...
            gearboundSentinel.Animator.speed = 1f;
            var hits = Physics2D.OverlapCircleAll(gearboundSentinel.transform.position, 1.5f, LayerMask.GetMask("Player")) // todo: config radius
            ...
            yield return new WaitForSeconds(gearboundSentinel.Animator.GetCurrentAnimatorClipInfo(0)[0].clip.length - HitTimeAfterUnburrowStart - CollisionTimeBeforeUnburrowEnd);
