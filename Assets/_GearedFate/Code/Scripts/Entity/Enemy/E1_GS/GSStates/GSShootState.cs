...
namespace BTG
{
    ...
    public class GSShootState : GSBaseState
	{
		...
		public GSShootState(FiniteStateMachine<GearboundSentinel.State> fsm, GearboundSentinel.State state, GearboundSentinel gs) : base(fsm, state,gs)
		{
		}
		...
		public override void OnEnter()
		{
			...
			gearboundSentinel.StartCoroutine(
				SprayCoroutine(gearboundSentinel.TargetPlayer.transform.position - shotCalculationOrigin.position, Random.Range(Mathf.Max(0, gearboundSentinel.Phase - 1), gearboundSentinel.Phase + 1)));
		}
		...
		public IEnumerator ShootProjectile(Vector2 direction, float windUp, Quaternion angleQuat)
		{
            ...
            GO.GetComponent<Rigidbody2D>().linearVelocity = direction.normalized * gearboundSentinel.ProjectileData.Speed;
        }

		IEnumerator SprayCoroutine(Vector2 centerDirection, int phasePattern)
		{
			int numBullets = phasePattern switch
			{
				0 => gearboundSentinel.Data.Stage0ShotgunProjCount,
				1 => gearboundSentinel.Data.Stage1ShotgunProjCount,
				_ => Mathf.RoundToInt(gearboundSentinel.Data.Stage2Spray360ProjPerRotation * gearboundSentinel.Data.Stage2Spray360NumRotations)
			};
			float spreadDeg = phasePattern switch
			{
			...
			float delayBetweenShots = gearboundSentinel.Phase switch // note: switches off phase, not phasePattern
            {
                0 => 0.4f,
            ...
            float rotPerShot = spreadDeg / (numBullets - 1);
			float startRot = Vector2.SignedAngle(Vector2.up, centerDirection) - spreadDeg / 2f;
			//if(circle)
            //    // remove temporary increase; the last bullet won't be spawned because that'll be the start of the circle
            //    numBullets--; 
            for (int i = 0; i < numBullets; i++)
			{
                Quaternion angleQuat = Quaternion.AngleAxis(startRot + rotPerShot * i, Vector3.forward);
				...
				yield return null; // TODO: Can we remove this?
				float wait = delayBetweenShots;
				float baseAnimationLength = gearboundSentinel.Animator.GetCurrentAnimatorClipInfo(0)[0].clip.length;
				...
				if (gearboundSentinel.AudioLaserShot)
					gearboundSentinel.AudioSource.PlayOneShot(gearboundSentinel.AudioLaserShot, gearboundSentinel.CalculateVolume(delayBetweenShots));
