...
namespace BTG
{
    ...
    public class GSDyingState : GSBaseState
	{
		...
		public GSDyingState(FiniteStateMachine<GearboundSentinel.State> fsm, GearboundSentinel.State state, GearboundSentinel gs) : base(fsm, state, gs)
		{
		}
		...
		public override void OnEnter()
		{
			...
			Component.Destroy(gearboundSentinel);
			GameManager.Instance.StartCoroutine(DelayedElevator()); // start it on GameManager because gearboundSentinel's coroutines will end right after destroy
		...
		IEnumerator DieAndStayDead()
		{
			...
			Vector3 pos = gearboundSentinel.transform.position;
			...
			while(true) // prevents other stuff from changing the animation
            {
				...
				float progress = played / animationLength;
