...
namespace BTG
{
   ...
   public class SteamCentipede : MonoBehaviour, IHealable, IBoss
   {
		...
		public float RegularSpeed { get; set; }

		[field:SerializeField]
		public float ChargeSpeed { get; set; }

		[field:SerializeField]
		public float DeathCircleSpeed { get; set; }

		[field:SerializeField]
		public float MinimumTimeBeforeCharge{ get; set; }

		[field:SerializeField]
		public float MaximumTimeBeforeCharge{ get; set; }

		[field:SerializeField]
		public float AccelerationWhenLosingBodyPart{ get; set; }
		...
		[field:SerializeField]
		public float ChargeBoundMultiplier{ get; set; }
		...
		protected void Start()
		{
			...
			for (int i = 0; i < _initialBodyPartsCount; i++)
			{
				GameObject bodypartGo = Instantiate(_bodyPartPrefab, transform, true);
			...
			GameObject trajectoryGo = Instantiate(_trajectoryPrefab);
			...
			_states.Add(CentipedeState.Chase, new CentipedeChaseState(_finiteStateMachine, Animator.StringToHash(nameof(CentipedeState.Chase)), this, ChasingDistance));
			...
			_states.Add(CentipedeState.Charge, new CentipedeChargeState(_finiteStateMachine, Animator.StringToHash(nameof(CentipedeState.Chase)), this));
			_states.Add(CentipedeState.DeathCircle, new CentipedeDeathCircleState(_finiteStateMachine, Animator.StringToHash(nameof(CentipedeState.Chase)), this));
			_states.Add(CentipedeState.Whip, new CentipedeWhipState(_finiteStateMachine, Animator.StringToHash(nameof(CentipedeState.Chase)), this));
			_states.Add(CentipedeState.Knocked, new CentipedeKnockedState(_finiteStateMachine, Animator.StringToHash(nameof(CentipedeState.Knocked)), this));
			_states.Add(CentipedeState.Dead, new CentipedeDeadState(_finiteStateMachine, Animator.StringToHash(nameof(CentipedeState.Dead)), this));
		...
		{
			Transform deadBodyPart = _bodyParts.First();
			UnregisterBodyPart();
		...
		{
			float3 tangent = _trajectory.EvaluateTangent(_currentPositionsOnSpline.Last());
			VectorHelper2D.Direction closestCardinal = VectorHelper2D.ClosestCardinal(new Vector2(tangent.x, tangent.y));
			return VectorHelper2D.VectorFromDirection(closestCardinal);
		}

		public void MoveAlongTrajectory()
		{
			float moved = Time.deltaTime * Speed;
		...
		{
			float lengthBeforeShorteningNode = _totalSplineLength;

			BezierKnot last = _trajectory.Spline.Knots.Last();
		...
		{
			for (int i = 0; i < _bodyParts.Count-1; i++)
			{
		...
		{
			Vector3 aimingPosition3D = new Vector3(aimingPosition.x, aimingPosition.y);
		...
		public void SetAnimations(int animId, bool isWalking)
		{
			for (int i = 0; i < _bodyParts.Count; i++)
			{
				float offset = isWalking ? (i % 2)/2f : 0f;
		...
		private void InitializeBodyPosition()
		{
			...
			// position head
			float headNormalizedDistance = _headDistance / _totalSplineLength;
			_bodyParts.Last().position = _trajectory.EvaluatePosition(normalizedDistance * (_bodyParts.Count()-2) + headNormalizedDistance);
