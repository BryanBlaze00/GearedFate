...
namespace BTG
{
   ...
   public class ObjectBreakAnim : MonoBehaviour
   {
      ...
      [SerializeField] private int _animSpritesCount = 7;

      [Tooltip("Index of the starting sprite in the animation sprite sheet")]
      [SerializeField] private int _animStartIndex = 0;
      ...
      private void Awake()
      {
         ...
         for  (int i = 0; i < _animSpritesCount; i++)
         {
      ...
      IEnumerator Explode()
      {
         ...
         for (int i = _animStartIndex; i < _animSpritesCount + _animStartIndex; i++)
         {
